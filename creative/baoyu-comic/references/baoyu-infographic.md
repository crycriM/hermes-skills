...
6. **No `image_generate` tool exists** — Step 6 references an `image_generate` tool. In practice, use one of these alternatives:
   - **matplotlib + PIL** (available on the system): best for technical diagrams, floor plans, cross-sections, engineering drawings, or any infographic where precise dimensions, annotations, and geometric accuracy matter. Use `terminal` to write and run Python scripts (NOT `execute_code` — its sandbox filesystem is isolated and files won't persist).
   - **AI image generation API** (if available via inference.sh or external): for artistic/conceptual infographics where visual style matters more than geometric precision.
7. **Technical diagrams → use matplotlib** — When the content is a plan, map, cross-section, blueprint, or anything requiring accurate dimensions and labeled annotations, AI image generators will hallucinate measurements. Use matplotlib instead with: `gaussian_filter1d` from scipy for smoothing organic shapes, progressive color fills for depth/heat maps, and `annotate` with `arrowstyle='<->'` for dimension lines.
8. **Cross-sections with depth << width** — Use vertical exaggeration (VSCALE multiplier on Y axis) for readability, and note it on the drawing. Without it, `set_aspect('equal')` produces extremely flattened images.
9. **`execute_code` vs `terminal` for file creation** — `execute_code` runs in a sandbox with its own filesystem. Files created via `write_file` inside `execute_code` do NOT appear in the real filesystem. For scripts and outputs that must persist, use `terminal("cat > file << 'EOF' ...")` to write files, then `terminal("python3 script.py")` to run them.
