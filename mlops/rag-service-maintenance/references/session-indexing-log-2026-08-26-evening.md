# Session Indexing Log — 2026-08-26 (evening)
...
- Service status: `active (running)`, Main PID 438228, started on first attempt — no restart counter (no crash-loop). Port 8001 bound cleanly.
- Health check `GET /health`: `{"embedding_model":"all-MiniLM-L6-v2","status":"ok"}` (embedding model loaded in ~5s — curl at +8s after restart succeeded).
