...
### General Operations
*   `hf download REPO_ID`: Download files from the Hub. Note: `--local-dir-use-symlinks` flag was removed in newer versions — just use `--local-dir`. Downloads land as `.incomplete` files in `<local-dir>/.cache/huggingface/download/` before being finalized.
