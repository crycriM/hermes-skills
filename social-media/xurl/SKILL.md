...
## Headless / No-Browser OAuth 2.0 Setup

When running on a headless machine (no display), `xurl auth oauth2` silently fails — it tries to open a browser and never prints the auth URL to stdout. You must perform the PKCE flow manually.

### Prerequisites
- xurl installed and app registered (`xurl auth apps add ...`)
- Redirect URI must use a free port (check with `ss -tlnp`). Common occupied ports: 8080 (llama-server), 8088 (open-webui), 8081 (RAG).
- Update the redirect URI in both the X developer portal AND the local app registration to match (e.g. `https://localhost:9999/callback`).

### Manual PKCE Flow

1. **Generate PKCE verifier + challenge + auth URL** using the `execute_code` tool with this Python pattern:
   - Generate `verifier` with `secrets.token_urlsafe(64)[:128]`
   - Generate `challenge` with `base64.urlsafe_b64encode(hashlib.sha256(verifier.encode()).digest()).rstrip(b'=').decode()`
   - Generate `state` with `secrets.token_urlsafe(32)`
   - Build auth URL: `https://twitter.com/i/oauth2/authorize?response_type=code&client_id=...&redirect_uri=...&scope=tweet.read+users.read+bookmark.read+like.read+offline.access&state=...&code_challenge=...&code_challenge_method=S256`
   - Save `{"verifier": verifier, "state": state}` to `/tmp/xurl_pkce.json`

2. **User opens the auth URL** on another device, authorizes the app, then copies the full redirect URL from the browser address bar. The page will fail to load (localhost not reachable) but the URL contains `?code=XXX&state=YYY`.

3. **Exchange the code for tokens** — POST to the X token endpoint with form data: `code`, `grant_type=authorization_code`, `client_id`, `redirect_uri`, `code_verifier` (from saved PKCE JSON). Response contains `access_token` and `refresh_token`.

4. **Write tokens to `~/.xurl`** — add `oauth2` section under the app entry with `access_token`, `refresh_token`, and `token_type`.

5. **Verify**: `xurl whoami`

### Pitfalls
- **OAuth 1.0a vs OAuth 2.0 credentials**: The X developer portal shows both. Consumer Key/Secret (OAuth 1.0a) will NOT work with `xurl auth apps add` — it requires `--client-id` and `--client-secret` (OAuth 2.0). The OAuth 2.0 Client ID typically ends in `...MTpjaQ`.
- **Port conflicts**: `xurl auth oauth2` starts a local HTTP listener on the redirect URI port. If port 8080 is occupied (llama-server, etc.), the flow fails with `bind: address already in use`. Use a free port and update both the portal and local registration.
- **xurl output capture**: When run in background or via process capture, `xurl auth oauth2` produces no stdout/stderr output even with a PTY. Don't waste time trying to capture the URL — use the manual PKCE flow above instead.
...
| `xurl auth oauth2` produces no output on headless | Binary tries to open browser directly, no fallback to printing URL | Use the manual PKCE flow (see "Headless / No-Browser OAuth 2.0 Setup" above) |
| `listen tcp 127.0.0.1:8080: bind: address already in use` | Redirect URI port occupied by another service | Re-register app with free port (e.g. 9999), update portal redirect URI |
...
| 401 despite `auth status` showing oauth2 user | xurl 1.1.0 bug: manually injected tokens visible but not used for auth header | Use the `xurl-read` wrapper at `~/.local/bin/xurl-read` that reads token from `~/.xurl` YAML and calls API directly |
